<?php
/**
 * Shared mobile bottom navigation for client view (viewMode === 'client').
 *
 * Expects:
...
<a <?php echo $hub_client_show; ?> href="<?php echo $hub_workflow_url; ?>" class="<?php echo esc_attr($hub_mobile_class); ?>">
...
<a <?php echo $hub_client_show; ?> href="#overview" @click="activeView = 'dashboard'" :class="activeView === 'dashboard' ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400'" class="flex flex-col items-center justify-center flex-1 h-full space-y-1 min-w-0">
    ...
    <span class="text-[10px]" :class="activeView === 'dashboard' ? 'font-medium' : ''">Overview</span>
</a>
<?php else : ?>
